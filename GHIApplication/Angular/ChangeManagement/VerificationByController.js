angular.module('GHIApp', []).controller('VerificationByController', function ($scope, $http) {
    ...
    function GetMasterInitialFn() {
        $scope.newData = { Id: 0, VerificationBy: sessionValue, VerificationDate: new Date(), VerificationComments: ""};
    }
    ...
    function GetDataFn() {
        ...
        $http.get('/api/ChangeApi/GetVerificationPendingList').then(function (r) {
            $scope.result = r.data;
    ...
    $scope.SubmitVerificationBy = function (x) {
        if (x != null) {
            $http.post('/api/ChangeApi/PostVerificationSave', x).then(function (r) {
                messageLoad(r.data);
                GetMasterInitialFn();
                GetDataFn();
