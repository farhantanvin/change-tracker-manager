angular.module('GHIApp', []).controller('RiskAuthorityController', function ($scope, $http) {
    ...
    function GetDataFn() {
        ...
        $http.get('/api/RiskAuthorityApi/Get').then(function (r) {
            $scope.result = r.data;
    ...
    $scope.SubmitFn = function (x) {
        if (x.Id === 0) {
            $http.post('/api/RiskAuthorityApi/Post', x).then(function (r) {
                if (r.data == 1) {
            ...
            $http.put('/api/RiskAuthorityApi/Put/' + x.Id, x).then(function (r) {
                messageLoad(r.data);
                GetInitialFn();
    ...
    $scope.StatusFn = function (x, y) {
        $scope.selectedRow = y;
        $("#btnSubmit").html('<i class="fa fa-save"></i> Save');
        $http.get('/api/RiskAuthorityApi/Get/' + x).then(function (r) {
            messageLoad(r.data);
            GetInitialFn();
            GetDataFn();
        });
    ...
    function removeData(x, y) {
        swal({
            ...
            function () {
                $http.delete('/api/RiskAuthorityApi/Delete/' + x).then(function (r) {
                    swal({
                        title: r.data,
                        type: "success",
                        showCancelButton: true,
