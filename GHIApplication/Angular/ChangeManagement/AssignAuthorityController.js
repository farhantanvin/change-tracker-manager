angular.module('GHIApp', []).controller('AssignAuthorityController', function ($scope, $http) {
    ...
    $http.get('/api/DropDownApi/GetCardNoData').then(function (r) {
        $scope.CardNoData = [];
        $scope.CardNoData.push({ Id: 0, CardNo: 'Select Card No' });
        if (r.data.length > 0)
    ...
    function GetDataFn() {
        ...
        $http.get('/api/AssignAuthorityApi/Get').then(function (r) {
            $scope.result = r.data;
    ...
    $scope.SubmitFn = function (x) {
        if (x.Id === 0) {
            $http.post('/api/AssignAuthorityApi/Post', x).then(function (r) {
                if (r.data == 1) {
            ...
            $http.put('/api/AssignAuthorityApi/Put/' + x.Id, x).then(function (r) {
                messageLoad(r.data);
                GetInitialFn();
    ...
    $scope.StatusFn = function (x, y) {
        $scope.selectedRow = y;
        $("#btnSubmit").html('<i class="fa fa-save"></i> Save');
        $http.get('/api/AssignAuthorityApi/Get/' + x).then(function (r) {
            messageLoad(r.data);
            GetInitialFn();
            GetDataFn();
        });
    ...
    function removeData(x, y) {
        swal({
            ...
            function () {
                $http.delete('/api/AssignAuthorityApi/Delete/' + x).then(function (r) {
                    swal({
                        title: r.data,
                        type: "success",
                        showCancelButton: true,
